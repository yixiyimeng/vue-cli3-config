...
export default {
  ...
  created () {
    ...
    this.localPagination = ['auto', true].includes(this.showPagination) && Object.assign({}, this.localPagination, {
	  ...
	  showTotal:function(total, range){ return `从第${range[0]}条到${range[1]}条，总计：${total}条`},
  ...
  methods: {
     ...
     */
    refresh (bool = true) {
      this.showPagination&&bool && (this.localPagination = Object.assign({}, {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          ...
          this.localPagination = Object.assign({}, this.localPagination, {
            current: r.pageNum?r.pageNum:((pagination && pagination.current) ||this.localPagination.current || this.pageNum), // 返回结果中的当前分页数
...
//console.log(this.localPagination);
      ...
      this.needTotalList = list.map(item => {
        return {
          ...
          total: selectedRows.reduce((sum, val) => {
            const total = sum + item.dataIndex;
  ...
  render () {
    ...
    const table = (
      <a-table {...{ props, scopedSlots: { ...this.$scopedSlots } }} onChange={this.loadData} bordered>
